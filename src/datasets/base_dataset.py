...
class BaseDataset(Dataset):
    ...
    def preprocess_data(self, instance_data):
        ...
        for key, tensor in instance_data.items():
            if isinstance(tensor, torch.Tensor) and tensor.ndimension() == 3:
                # Permute the tensor from (H, W, C) to (C, H, W)
                instance_data[key] = tensor.permute(2, 0, 1)
        return instance_data
